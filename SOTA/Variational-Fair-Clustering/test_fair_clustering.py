...
def  main(args):
    ...
    X_org = np.loadtxt(r'../../Datasets/subsampled_adult.txt', delimiter=',')
    demograph = np.loadtxt(r'../../Datasets/subsampled_adult_Color.txt')
    K = 5
...
#    J = len(V_sum)
    ...
    for count,lmbda in enumerate(lmbdas):
        ...
        C,l,elapsed,S,E = fair_clustering(X, K, u_V, V_list, lmbda, args.L, fairness, cluster_option, C_init=C_init, l_init=l_init)
        # save to .txt
        # np.savetxt('adult_VFC.txt', l+1, fmt='%d')
...
if __name__ == '__main__':
    ...
    #Lambda
    parser.add_argument('--lmbda', type=float, default=9000) # specified lambda
    parser.add_argument('--lmbda-tune', type=str2bool, default=True)  # run in a range of different lambdas
    parser.add_argument('--L', type=float, default= 1.0)  # Lipchitz value in bound update
