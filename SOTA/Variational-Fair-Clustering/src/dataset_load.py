...
def read_dataset(name, data_dir):
    ...
    if name not in __datasets:
        ...
        label_value_list = list(set(label_str.astype(str).values))
        label[label_str.astype(str).values == label_value_list[0]] = 1
        label[label_str.astype(str).values == label_value_list[1]] = 2
        

        #dropping non-numerical features and normalizing data
        cont_types = [0,2,4,9,10,11]
        df = df.iloc[:,cont_types]
        data = np.array(df.values, dtype=float)
        
        data = data[:,[0,1,2,3,5]]

        # np.savetxt(r'C:\Users\rebpan\TII_Fair_Code\Datasets\adult.txt', data, fmt='%g', delimiter=',')
        # np.savetxt(r'C:\Users\rebpan\TII_Fair_Code\Datasets\adult_label.txt', label, fmt='%d')
        # np.savetxt(r'C:\Users\rebpan\TII_Fair_Code\Datasets\adult_Color.txt', sex_num, fmt='%d')
        # pass
        #Scale data
...
#        df = df.loc[np.random.choice(df.index, n, replace=False)]
        ...
        df = df[['age', 'duration', 'campaign', 'cons.price.idx', 'euribor3m', 'nr.employed']].values
        ...
        np.savetxt(r'C:\Users\rebpan\TII_Fair_Code\Datasets\bank.txt', data, fmt='%g', delimiter=',')
        np.savetxt(r'C:\Users\rebpan\TII_Fair_Code\Datasets\bank_label.txt', label, fmt='%d')
        np.savetxt(r'C:\Users\rebpan\TII_Fair_Code\Datasets\bank_Color.txt', sex_num, fmt='%d')
        pass
        ...
        data = np.array(df, dtype=float)
        # data = scale(data, axis = 0)    
        np.savetxt(r'C:\Users\rebpan\TII_Fair_Code\Datasets\census1990.txt', data, fmt='%g', delimiter=',')
        np.savetxt(r'C:\Users\rebpan\TII_Fair_Code\Datasets\census1990_Color.txt', sex_num, fmt='%d')
        pass
